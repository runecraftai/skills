...
describe("grimoire CLI", () => {
  test("lists categorized catalog entries", () => { const r = run(["list"]); expect(r.status).toBe(0); expect(r.stdout).toContain("[Build & Design]"); expect(r.stdout).toContain("spec-driven"); });
